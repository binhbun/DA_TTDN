...
function dataInsertTransaction($dataReq, $user): array
{
    return array(
        'order_code' => $dataReq['orderCode'],
        'amount' => $dataReq['amount'],
        'status' => $dataReq['status'] ?? 1,
        'user_id' => $user['id'] ?? 0,
    );
}
...
{
    $data = strtolower($data); // Chuyển đổi chuỗi thành chữ thường
    ...
    else if ($data == 'garena')
    {
        ...
        // Xử lý khi giá trị không khớp với bất kỳ tùy chọn nào ở trên
        return 'Không tìm thấy thông tin thẻ.';
    }
